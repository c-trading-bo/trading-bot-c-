...
public class StructuredConfigurationManager
{
    ...
    public StructuredConfigurationManager(ILogger<StructuredConfigurationManager> logger, List<string> configPaths)
    {
        _logger = logger;
    ...
    private ConfigurationSource CreateDefaultConfigurationSource(string strategy)
    {
        return new ConfigurationSource
        {
            ...
            {
                ["DefaultMultiplier"] = 1.0m,
                ["DefaultThreshold"] = 0.65m,
                ["MaxPositionSize"] = 5,
                ["MinConfidence"] = 0.6m
            },
