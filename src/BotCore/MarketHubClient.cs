...
{
    public sealed class MarketHubClient(ILogger<MarketHubClient> log, Func<Task<string?>> getJwtAsync) : IAsyncDisposable
    {
        private readonly ILogger<MarketHubClient> _log = log;
        private readonly Func<Task<string?>> _getJwtAsync = getJwtAsync;
        ...
        private async Task SubscribeIfConnectedAsync(CancellationToken ct)
        {
            ...
            try
            {
                ...
                async Task Call(string method, string cid)
                {
                    ...
                    {
                        _log.LogWarning(ex, "[MarketHub] {Method}({Cid}) FAILED: {Error}", method, cid, ex.Message);
                    }
                }
        ...
        private async Task<HubConnection> BuildMarketHubAsync()
        {
            ...
            // Include access_token in querystring only when RTC_URL_TOKEN not disabled
            var allowUrlToken = (Environment.GetEnvironmentVariable("RTC_URL_TOKEN") ?? "1").Trim().ToLowerInvariant() is "1" or "true" or "yes";
            var urlWithToken = (!allowUrlToken || string.IsNullOrWhiteSpace(jwt)) ? url : $"{url}?access_token={Uri.EscapeDataString(jwt)}";
            var hub = new HubConnectionBuilder()
                .WithUrl(urlWithToken, o =>
                {
                    o.AccessTokenProvider = _getJwtAsync; // header provider (preferred)
