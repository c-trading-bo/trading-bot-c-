    ...
    public class StreamingFeatureAggregator : IDisposable
    {
        ...
        public async Task<StreamingFeatures> ProcessTickAsync(MarketTick tick, CancellationToken cancellationToken)
        {
            try
            {
                ...
                // Generate features
                var features = await GenerateFeaturesAsync(tick.Symbol, tick.Timestamp, cancellationToken).ConfigureAwait(false).ConfigureAwait(false);
        ...
        public async Task<StreamingFeatures?> GetCachedFeaturesAsync(string symbol)
        {
            ...
            return await GenerateFeaturesAsync(symbol, latestTick.Timestamp, CancellationToken.None).ConfigureAwait(false).ConfigureAwait(false);
        }
