...
{
    /// <summary>
    /// ONNX ensemble engine with confidence voting, async batched inference, and GPU support
    /// Implements proper session disposal and memory management
    /// </summary>
    public class OnnxEnsembleService : IDisposable
    {
        ...
        public OnnxEnsembleService(
            ILogger<OnnxEnsembleService> logger,
            IOptions<OnnxEnsembleOptions> options)
        {
            ...
            _logger.LogInformation("🧠 ONNX Ensemble Service initialized with {MaxBatchSize} batch size, {MaxConcurrentBatches} concurrent batches",
        ...
        public async Task<bool> LoadModelAsync(string modelName, string modelPath, double confidence = 1.0, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                _logger.LogInformation("✅ Model loaded: {ModelName} with confidence {Confidence:F2}", modelName, confidence);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> UnloadModelAsync(string modelName, CancellationToken cancellationToken = default)
        {
            try
            {
                if (_modelSessions.TryRemove(modelName, out var modelSession))
                {
                    ...
                    _logger.LogInformation("🗑️ Model unloaded: {ModelName}", modelName);
                    return true;
                }
                return false;
            }
        ...
        public async Task<EnsemblePrediction> PredictAsync(float[] features, CancellationToken cancellationToken = default)
        {
            ...
            {
                _logger.LogWarning("⚠️ Anomalous input detected and blocked");
                return new EnsemblePrediction
                {
                    IsAnomaly = true,
                    Confidence = 0.0,
                    Predictions = new Dictionary<string, ModelPrediction>(),
                    EnsembleResult = 0.0f
                };
            }
        ...
        private async Task ProcessBatchAsync(List<InferenceRequest> batch)
        {
            try
            {
                ...
                _logger.LogDebug("🔮 Processed batch of {BatchSize} in {LatencyMs:F2}ms", batch.Count, totalLatency);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "❌ Error processing inference batch");
        ...
        private async Task<EnsemblePrediction[]> RunEnsembleInferenceAsync(float[][] batchFeatures)
        {
            ...
            var modelTasks = modelSessions.Select(async modelSession =>
            {
                ...
                {
                    _logger.LogError(ex, "❌ Error in model inference: {ModelName}", modelSession.Name);
                }
            }).ToArray();
        ...
        private SessionOptions CreateSessionOptions()
        {
            ...
            if (_options.UseGpu)
            {
                try
                {
                    ...
                    _logger.LogInformation("🚀 GPU acceleration enabled for ONNX inference");
                }
                catch (Exception ex)
                {
                    _logger.LogWarning(ex, "⚠️ Failed to enable GPU acceleration, falling back to CPU");
                }
            }
        ...
        private async Task ValidateModelAsync(ModelSession modelSession, CancellationToken cancellationToken)
        {
            try
            {
                ...
                _logger.LogDebug("✅ Model validation passed: {ModelName} - Input: {InputShape}, Output: {OutputShape}",
                    modelSession.Name, string.Join("x", inputInfo.Value.Dimensions), string.Join("x", outputInfo.Value.Dimensions));
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "❌ Model validation failed: {ModelName}", modelSession.Name);
                throw;
            }
        }
        ...
        public void Dispose()
        {
            if (!_disposed)
            {
                ...
                {
                    _logger.LogWarning(ex, "⚠️ Timeout waiting for batch processing task to complete");
                }
                ...
                _logger.LogInformation("🛑 ONNX Ensemble Service disposed");
            }
        }
    }

    /// <summary>
    /// Configuration options for ONNX ensemble service
    /// </summary>
    ...
    /// <summary>
    /// Simple anomaly detector using statistical thresholds
    /// </summary>
